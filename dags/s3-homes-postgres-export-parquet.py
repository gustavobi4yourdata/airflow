...
# declare dag
@dag(
    dag_id="s3-homes-postgres-export-parquet",
...
def export_file():
    ...
    load_vehicle_data = aql.load_file(
        ...
        input_file=File(path="s3://landing/homes/homes", filetype=FileType.CSV, conn_id=S3_CONN_ID),
        output_table=Table(name="homes", conn_id=POSTGRES_CONN_ID, metadata=Metadata(schema="astro")),
        if_exists="replace",
    ...
    aql.export_to_file(
        ...
        output_file=File(path="s3://processing/homes/homes.parquet"),
        if_exists="replace",
    )
