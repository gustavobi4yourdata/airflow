...
# declare dag
@dag(
    dag_id="s3-json-informacoes-postgres.py",
...
def elt_with_transforms():
    ...
    # users group
    with TaskGroup(group_id="Informacoes") as stripe_task_group:

        load_s3_to_database = aql.load_file(
            task_id="load_s3_to_database",
            input_file=File(path="s3://landing/informacoes/informacoes", filetype=FileType.JSON, conn_id=S3_CONN_ID),
            output_table=Table(name="informacoes", conn_id=POSTGRES_CONN_ID, metadata=Metadata(schema="astro")),
            if_exists="replace",
