...
# declare dag
@dag(
    dag_id="s3-json-subscription-postgres",
...
def elt_with_transforms():
    ...
    # users group
    with TaskGroup(group_id="Subscription") as stripe_task_group:

        load_s3_to_database = aql.load_file(
            task_id="load_s3_to_database",
            input_file=File(path="s3://landing/subscription/subscription_2023_1_18", filetype=FileType.JSON, conn_id=S3_CONN_ID),
